...
*  algorithms for a large k. Let us first limit k. Let k <= nc where c is a
*  constant. In that case, the complexity becomes O(nLogb(n)). But it still
*  doesn’t beat comparison-based sorting algorithms.
What if we make the value of b larger?. What should be the value of b to make
* the time complexity linear? If we set b as n, we get the time complexity as
* O(n). In other words, we can sort an array of integers with a range from 1 to
*  nc if the numbers are represented in base n (or every digit takes log2(n) bits).
...
Following is a simple implementation of Radix Sort. For simplicity, the value
*  of d is assumed to be 10. We recommend you to see Counting Sort for details
* of countSort() function in below code.*/
