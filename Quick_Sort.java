...
ow  --> Starting index,  high  --> Ending index
...
* given in CLRS book. The logic is simple, we start from the leftmost element
* and keep track of index of smaller (or equal to) elements as i. While
...
* element as pivot. Following is recurrence for best case.
 ...
 T(n) = 2T(n/2) + \theta(n)
The solution of above recurrence is \theta(nLogn). It can be solved using case
* 2 of Master Theorem.
...
* sorting algorithm as it uses extra space only for storing recursive function
...
*  {1, 4, 2, 4, 2, 4, 1, 2, 4, 1, 2, 2, 2, 2, 4, 1, 4, 4, 4}. If 4 is
*  picked as pivot in Simple QuickSort, we fix only one 4 and recursively
* process remaining occurrences. In 3 Way QuickSort, an array arr[l..r] is
*  divided in 3 parts:
...
*  average complexity we find that both type of sorts have O(NlogN) average
*  complexity but the constants differ. For arrays, merge sort loses due to
...
* be adjacent in memory. Unlike array, in linked list, we can insert items in the
* middle in O(1) extra space and O(1) time. Therefore merge operation of merge sort
* can be implemented without extra space for linked lists.

In arrays, we can do random access as elements are continuous in memory. Let us say
*  we have an integer (4-byte) array A and let the address of A[0] be x then to
* access A[i], we can directly access the memory at (x + i*4). Unlike arrays, we
* can not do random access in linked list. Quick Sort requires a lot of this kind
* of access. In linked list to access i’th index, we have to travel each and every
* node from the head to i’th node as we don’t have continuous block of memory.
* Therefore, the overhead increases for quick sort. Merge sort accesses data
